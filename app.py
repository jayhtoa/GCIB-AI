...
2. 支援三語：自動使用廣東話（預設）、英文或普通話回覆。

【真實地點與交通】
- 本工廠地址：土瓜灣落山道 108 號。
- 港鐵站：港鐵土瓜灣站 B 出口。
- 巴士線：5C、11X、21、26、85X 等。
...
if st.session_state.admin_mode:
    ...
    if admin_audio is not None:
        with st.spinner("🎙️ 正在辨識語音中..."):
            try:
                ...
                # 將語音辨識結果填入 session_state，讓下方的 text_area 讀取
    ...
    if issue_text.strip():
        ...
        wa_url = f"https://wa.me/85223646837?text={encoded_msg}"
        sms_url = f"sms:+85223646837?body={encoded_msg}"
        
        c1, c2 = st.columns(2)
        with c1:
            st.link_button("📲 發送 WhatsApp 畀管業處", wa_url, use_container_width=True)
        with c2:
            st.link_button("✉️ 發送 SMS 畀管業處", sms_url, use_container_width=True)
    else:
        st.warning("💡 請輸入或錄音講出問題，系統會為你產生 WhatsApp 及 SMS 發送按鈕。")
